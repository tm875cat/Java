...
public class shopCarTest {
    ...
    public static void add(Goods[] shopCar, Scanner sc) {
        ...
        System.out.println("商品"+g.getName()+"加入購物車");
    }
    ...
    public static void update(Goods[] shopCar, Scanner sc) {
        ...
        while (true) {
            ...
            if (g == null && id != 0) {
                ...
                System.out.println("請輸入" + g.getName() + "欲修改的購買數量");
                int buyquantity = sc.nextInt();
                g.setQuantity(buyquantity);
    ...
    public static void search(Goods[] shopCar) {
        ...
        for (int i = 0; i < shopCar.length; i++) {
            Goods g = shopCar[i];
            if (g != null) {
                System.out.println(g.getId() + "\t\t" + g.getName() + "\t\t\t" + g.getQuantity() + "\t\t" + g.getPrice());
    ...
    public static void pay(Goods[] shopCar) {
        ...
        for (int i = 0; i < shopCar.length; i++) {
            Goods g = shopCar[i];
            if (g != null) {
                money += (g.getPrice() * g.getQuantity());
            } else {
                break;
            }
        }
